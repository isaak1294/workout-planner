interface MuscleSVGProps {
  selectedMuscle: string | null;
  onSelect: (muscle: string) => void;
}

const Fascia: React.FC<MuscleSVGProps> = ({ selectedMuscle, onSelect }) => (
  <g
    id="Fascia"
    transform='translate(2, 0)'
    className={`fill-gray-200 stroke-gray-800 stroke-2 transition-colors duration-200 ${
      selectedMuscle === 'fascia' ? 'fill-red-400' : ''
    } hover:fill-red-200 cursor-pointer`}
    onClick={() => onSelect('fascia')}
    role="button"
    aria-label="Fascia"
  >
  <g>
    <path id="Thoracolumbar_00000052827203067967247980000013786644313683635124_" className="st1" strokeWidth={0.1} d="M226.63,279.59
      c2.62,2.06,1.86,5.85,1.61,6.92c-0.55,2.3-11.74,16.77-15.62,20.55c-0.3,0.3-1.33-0.04-1.64-0.32
      c-4.18-3.87-15.42-18.03-15.97-20.33c-0.26-1.07-0.34-4.66,2.33-6.74c3.79-1.81,4.58-9.42,5.42-12.33
      c2.21-7.61,3.96-15.2,5.06-23.11c0.86-1.77,2.44-4.12,3.29-5.23c0.6-0.79,1.21-0.67,1.76,0.12c0.73,1.04,2.2,3.49,3.05,5.31
      c1.11,7.76,3.02,15.45,5.19,22.93C221.95,270.21,222.95,277.66,226.63,279.59z"/>
    <path className="st2" strokeWidth={0.1} d="M211.76,283.53c0.11,0,0.26-0.15,0.36-0.5c1.3-5.69,9.05-14.92,9.13-15.02l-0.38-0.7
      c-0.31,0.36-6.68,7.99-8.9,13.76c-0.08,0.21-0.36,0.21-0.44,0c-2.21-5.78-8.66-13.51-8.96-13.87l-0.32,0.78
      c0.08,0.1,7.85,9.37,9.13,15.05C211.47,283.36,211.56,283.54,211.76,283.53z"/>
    <path className="st2" strokeWidth={0.1} d="M228.2,282.18l-0.25-0.78c-0.12,0.14-11.55,13.46-15.84,18.83c-0.1,0.12-0.27,0.12-0.37,0
      c-4.28-5.36-15.84-18.89-15.96-19.03l-0.3,0.71c0.12,0.14,14.48,17.37,16.26,19.51c0.1,0.12,0.27,0.12,0.36,0
      C213.88,299.29,228.09,282.31,228.2,282.18z"/>
    <path className="st2" strokeWidth={0.1} d="M211.3,255.34c0.17,0.24,0.28,0.35,0.46,0.34c0.18-0.01,0.28-0.16,0.45-0.34c2.25-3.01,4.72-4.6,4.75-4.62
      l-0.39-0.61c-0.11,0.07-2.39,1.54-4.62,4.38c-0.1,0.12-0.29,0.12-0.38,0c-2.24-2.87-4.54-4.35-4.65-4.42l-0.39,0.61
      C206.56,250.7,209.04,252.31,211.3,255.34z"/>
    <path className="st2" strokeWidth={0.1} d="M211.59,246.77c0.09,0.08,0.23,0.08,0.33,0c2.11-1.86,3.88-3.19,3.88-3.19l-0.47-0.55c0,0-1.53,1.12-3.39,2.76
      c-0.09,0.08-0.24,0.08-0.33,0c-1.95-1.73-3.26-2.95-3.26-2.95l-0.47,0.55C207.86,243.38,210.86,246.11,211.59,246.77z"/>
    <path className="st2" strokeWidth={0.1} d="M211.35,262.35c0.12,0.25,0.21,0.41,0.42,0.41c0.21,0,0.32-0.24,0.38-0.41c2.45-5.22,5.65-6.25,5.68-6.26
      l-0.21-0.7c-0.14,0.04-3.15,0.98-5.66,5.69c-0.09,0.17-0.34,0.17-0.43,0c-2.53-4.74-5.64-5.74-5.78-5.78l-0.21,0.7
      C205.59,256.01,208.9,257.11,211.35,262.35z"/>
    <path className="st2" strokeWidth={0.1} d="M211.36,271.21c0.14,0.33,0.16,0.51,0.4,0.51c0.23,0,0.24-0.12,0.4-0.51c2.44-5.73,7.31-10.06,7.36-10.1
      l-0.48-0.55c-0.05,0.04-4.45,3.94-7.06,9.28c-0.09,0.18-0.34,0.18-0.43,0c-2.62-5.36-7.03-9.27-7.08-9.32l-0.48,0.55
      C204.03,261.12,208.92,265.46,211.36,271.21z"/>
    <path className="st2" strokeWidth={0.1} d="M211.7,293.5c0.16,0,0.17-0.05,0.26-0.21c2.48-4.73,11.71-16.68,11.8-16.79l-0.46-0.65
      c-0.37,0.47-8.46,10.93-11.34,16c-0.09,0.17-0.32,0.17-0.41,0c-2.86-5.05-11-15.49-11.36-15.96l-0.45,0.58
      c0.1,0.12,9.69,12.46,11.75,16.82C211.49,293.27,211.57,293.5,211.7,293.5z"/>
    <path id="brown_00000003085666028766288340000016579337965886009731_" className="st3" strokeWidth={0.1} d="M197.22,279.3
      c-0.77,2.69,8.65,25.41,14.49,25.49c5.04,0.07,15.61-22.09,13.95-25.94c1.15,0.65,2.12,1.95,2.52,2.8
      c0.42,0.89,0.54,2.95,0.34,3.91c-0.3,1.45-2.83,5.21-2.83,5.21l-5.37,7.27l-5.31,6.43c0,0-2.59,3.1-3.3,3.02
      c-0.42-0.04-6.71-6.95-6.71-6.95l-5.13-6.46c0,0-3.05-3.91-3.88-5.33c-0.38-0.66-1.08-2.04-1.25-2.79
      c-0.17-0.76-0.3-2.36-0.08-3.11C194.97,281.8,196.29,279.88,197.22,279.3z"/>
  </g>
  <g>
    <path className="st4" strokeWidth={0.1} d="M195.15,281.39L195.15,281.39c0.18,0.22,1.64,1.97,3.66,4.4c-0.25-0.66-0.47-1.29-0.66-1.88
      c-1.23-1.48-2.15-2.58-2.53-3.03C195.35,281.17,195.15,281.39,195.15,281.39z"/>
    <path className="st4" strokeWidth={0.1} d="M228.18,281.73c0,0-0.12-0.14-0.3-0.36c-0.42,0.48-1.58,1.83-3.11,3.61c-0.2,0.6-0.42,1.21-0.66,1.85
      c2.11-2.47,3.72-4.34,4.1-4.78L228.18,281.73z"/>
  </g>

  </g>
)

export default Fascia;